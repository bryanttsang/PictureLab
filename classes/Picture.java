...
public class Picture extends SimplePicture 
{
  ...
  public void fixUnderwater()
  {
    ...
    for (Pixel[] rowArray : pixels)
    {
      for (Pixel pixelObj : rowArray)
      {
        ...
        g = pixelObj.getGreen();
        b = pixelObj.getBlue();
        if (pixelObj.getRed() <= 127) { r = r * 2; }
        if (pixelObj.getRed() >= 128) { r = r / 2; }
        if (pixelObj.getGreen() <= 127) { g = g * 2; }
        if (pixelObj.getGreen() >= 128) { g = g  / 2; }
        if (pixelObj.getBlue() <= 127) { b = b * 2; }
        if (pixelObj.getBlue() >= 128) { b = b  / 2; }
        pixelObj.setRed(r);
        pixelObj.setGreen(g);
        pixelObj.setBlue(b);
      }
  ...
  public int[] getAverageForColumn(int col)
  {
    ...
    {
      pixel = pixels[row][col];
      average[row] = ( pixel.getRed()+pixel.getGreen()+pixel.getBlue() ) /3;
    }
    return average;
  }
